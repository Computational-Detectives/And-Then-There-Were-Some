...
from src.manual_extraction.config import (
    RAW_TEXT, CLEAN_NAMES, 
    OUT_DIR, TOKENS, SPACY_MODEL
    )

from src.manual_extraction.utils import (
...
def ensure_model_server():
    ...
    if _server_is_up():
        print_information("Model server already running.", prefix="    ")
        return
...
def main(args: argparse.Namespace):
    ...
    if stages_set.intersection({0, 1, 3, 4}):
        if args.debug:
            print_information("Debug mode: ensuring model server is running...", prefix="")
            ensure_model_server()
    ...
    if 0 in stages:
        t0 = time.time()
        from src.manual_extraction.tokenise import main as tokenise
    ...
    if 1 in stages:
        t0 = time.time()
        from src.manual_extraction.character_resolution import main as character_resolution
    ...
    if 3 in stages:
        t0 = time.time()
        from src.manual_extraction.verb_extraction import main as verb_extraction
        verb_extraction(
            tokens_path=tokens_path,
    ...
    if 4 in stages:
        t0 = time.time()
        from src.manual_extraction.object_extraction import main as object_extraction
        object_extraction(
            text_path=text_path,
            tokens_path=tokens_path,
            coref_dir=out_dir / "coreference",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Run coreference resolution concurrently.",
    )
