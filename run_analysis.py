...
from src.config import (
    BASE_DATA_DIR, BASE_OUT_DIR,
    COOC_OUT, TRIPLE_OUT, EGO_OUT, TOKENS,
)
...
# ─────────────────────────────────────────────
# Timeline (character_timeline.py + timeline_data.py)
# ─────────────────────────────────────────────
...
def run_timeline(args: argparse.Namespace) -> None:
    ...
    victims = tl_load_victims(BASE_DATA_DIR, BASE_OUT_DIR)
    ...
    if args.hierarchy:
        hier_path = (
            ...
            else Path(COOC_OUT) / "raw_cooccurrences.csv"
        )
...
def run_ego(args: argparse.Namespace) -> None:
    ...
    # Load victims
    victims = ego_load_victims(BASE_DATA_DIR, BASE_OUT_DIR)
...
if __name__ == "__main__":
    ...
    all_parser.add_argument(
        "-i", "--input", type=Path,
        default=Path(f"{TRIPLE_OUT}/combined_triples.csv"),
        help="Path to triples TSV — used for ego-network and hierarchy",
    )
    all_parser.add_argument(
        "-o", "--out", type=Path, default=Path(EGO_OUT),
        help="Base output directory (timeline writes to <out>/timeline)",
    )
    # Ego-specific
    all_parser.add_argument(
        "-f", "--format", choices=["triples", "edges"], default=None,
    ...
    all_parser.add_argument(
        "-b", "--bins", type=int, default=50,
        help="Timeline: number of time bins (default: 50)",
    )
    all_parser.add_argument(
        "-m", "--bin-mode",
        choices=["sentences", "normalised", "chapters", "all"],
        default="all",
        help="Timeline: binning mode (default: all)",
    )
    all_parser.add_argument(
        "-t", "--token-range", type=int_pair, default=None,
        help="Timeline: restrict to token-ID range, e.g. '3780,7540'",
    )
    all_parser.add_argument(
        "-c", "--characters", type=str, default=None,
        help="Timeline: comma-separated character name filter",
    )
    all_parser.add_argument(
        "--timeline-input", type=Path,
        default=Path(f"{COOC_OUT}/raw_occurrences.csv"),
        help="Timeline: occurrence input file (default: raw_occurrences.csv)",
    )
    # Accept these so users can pass them without error (hierarchy is
    # auto-enabled in 'all' mode; --hierarchy-input overrides -i for the
    # timeline hierarchy if provided).
    all_parser.add_argument(
        "--hierarchy", action="store_true", default=True,
        help="(Always on in 'all' mode — accepted for compatibility)",
    )
    all_parser.add_argument(
        "--hierarchy-input", type=Path, default=None,
        help="Override hierarchy input (defaults to the -i triples file)",
    )
